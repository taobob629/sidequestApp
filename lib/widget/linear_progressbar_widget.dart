        ...
        builder: (context, double value, widget) {
          return  RepaintBoundary(
            child: CustomPaint(
              ...
              painter: ProgressPaint(progress: progress, width: width, height: height),
            ),
          );
        },
      );
...
class ProgressPaint extends CustomPainter {
  ...
  ProgressPaint({required this.progress, required this.width, required this.height});

  var gradient = LinearGradient(
    colors: [Color(0xFFED5A24), Color(0xFFCFAB21)],
  );

  @override
  void paint(Canvas canvas, Size size) {
    if (progress > 0) {
      drawPaint.shader = gradient.createShader(Rect.fromLTWH(0, 0, size.width / 2, height));
    }
