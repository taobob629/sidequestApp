...
import 'package:sq_hub_app/widget/pages.dart' as p;
...
mixin CupertinoRouteTransitionMixin<T> on p.PageRoute<T> {
  ...
  static Widget buildPageTransitions<T>(
    ...
    // if (RouteDirectionState.routeIsFromDown) {
        ...
        if (RouteState.isDrawer) {
          return ScaleTransition(
            scale: (linearTransition
                ...
                .drive(Tween(begin: 1.0, end: RouteState.scaleValue)),
  ...
  if (isMove) {
    if (isClose) {
      return Navigator.pushAndRemoveUntil(
        c ?? (isMobile ? context : context)!,
        CupertinoPageRoute(
          builder: (_) => SafeArea(top: false, child: p, bottom: false),
        ),
        (v) => v == null,
      );
    } else {
      return Navigator.push(
        ...
        CupertinoPageRoute(
          builder: (_) {
            return SafeArea(top: false, child: p, bottom: false);
          },
        ),
      ).then((v) async {
        await callback!(v);
    ...
    if (isClose) {
      ...
      return Navigator.pushAndRemoveUntil(
        ...
        //   Container(
        //     color: Colors.white,
        //     child: SafeArea(top: false, child: p,bottom: false),
        //   ),
        //   isMove: false,
        //   isMoveBtm: false,
        // ),
