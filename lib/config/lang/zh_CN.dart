const Map<String, String> zh_CN = {
  'Sign In': '登录',
  'SIGN IN': '登录',
  'Account Email / Member ID': 'Account Email / Member ID',
  'Password': '密码',
  'Sign Up': '注册',
  'Forgotten your password?': '忘记密码?',
  'Welcome back': '欢迎回来',
  'Sign in with your SideQuest ID or email.': '使用 SideQuest ID 或邮箱登录',
  'SideQuest ID / Email': 'SideQuest ID / 邮箱',
  'Enter your ID or email': '请输入 ID 或邮箱',
  'Enter your password': '请输入密码',
  'Forgot password?': '忘记密码？',
  'New to SideQuest?': '第一次使用 SideQuest？',
  'Create account': '创建账户',
  'or continue with': '或使用以下方式登录',
  'Continue with Google': '使用 Google 登录',
  'Continue with Apple': '使用 Apple 登录',
  'Please input your email': '请输入Email',
  'Please input your password': '请输入密码',
  'Account Validation': '账户验证',
  'Validate by': 'Validate by',
  'Validate information': '验证信息',
  'Info：': '提示',
  'After applying for an appointment, please go to the SideKick homepage to enter the SideKick Official DC. Wait in the assessment channel 5minutes before the assessment time, if you are not in the DC at the stipulated time, the appointment will be canceled automatically.': '请陪玩申请预约时间后自行到SideKick首页进入SideKick官方DC. 请在审核时间前5分钟在SideKick官方DC的审核频道等待，如在规定时间不在DC，将自动作废此次预约.',
  'CONFIRM': '确认',
  'Please input your': '请输入',
  'Terms and Conditions': '条款和条件',
  'Privacy Policy': '隐私政策',
  'SideKick Policy': 'SideKick 协议',
  'User Agreement': '用户协议',
  'By checking this means you agree to our': '我已阅读并同意',
  'Update Profile': '编辑资料',
  'Profile Information': '个人资料',
  'Account Email': 'Email',
  'Guardian Email': '监护人邮箱',
  'Players under the age of 16 must provide an emergency contact in order to use our services and sign up.':
  'Players under the age of 16 must provide an emergency contact in order to use our services and sign up.',
  'SEND VERIFICATION CODE': '发送验证码',
  'Verification code from your email': '电子邮件中的验证码',
  'First Name': '名字',
  'Last Name': '姓氏',
  'Nick Name': '昵称',
  'Male': '男',
  'Female': '女',
  'Phone Number': '电话号码',
  'Login Password': '登录密码',
  'Payment Pin': '支付密码',
  'Invite Code (Optional)': '邀请码 (可选)',
  'UPDATE': '更新',
  'Please input a email as your account': '请输入电子邮件作为您的帐户',
  'Please input a valid email': '请输入有效的邮箱',
  'Players under the age of 13 will not be able to signup for our services, instead a parent must make the account on their behalf':
  '13岁以下的玩家将无法注册我们的服务，家长必须代表其创建账户',
  'Please input your guardian email': '请输入您的监护人电子邮件',
  'Please input a valid guardian email': '请输入有效的监护人电子邮件',
  'Guardian email cannot be the same as your account':
  'Guardian email cannot be the same as your account',
  'Verification code sent': '验证码已发送',
  'Players under the age of 13 will not be able to signup for our services, instead a parent must make the account on their behalf.':
  '13岁以下的玩家将无法注册我们的服务，家长必须代表其创建账户。',
  'Please input your verification code': '请输入验证码',
  'Password no less than 6 characters': '密码不少于6个字符',
  'Please input your first name': '请输入您的名字',
  'Please input your last name': '请输入你的姓氏',
  'Please input your nick name': '请输入您的昵称',
  'Please input your phone number': '请输入您的电话号码',
  'Only 6 numbers accepted as your payment pin': '支付密码只接受6位数字',
  'Congratulations and welcome, please sign in with your new account!':
  '恭喜并欢迎您，请使用新账户登录！',
  'Congratulations and welcome, your profile has been updated!':
  '恭喜并欢迎您，您的个人资料已更新！',
  'Authorization': '授权',
  'You are signing in to a PC client with account below, do you want to continue?':
  '您正在使用以下账户登录PC客户端，您是否要继续？',
  'Success': '成功',
  'Retrieve': '找回密码',
  'PASSWORD': '密码',
  'PIN': 'PIN',
  'Your sign in account email': '您的登录帐户电子邮件',
  'New': '新',
  'password': '密码',
  'pin': 'pin',
  '* Only 6 numbers accepted as your payment pin':
  '* 支付密码只接受6位数字',
  'Your': '您的',
  'has been successfully reset!': '已成功重置！',
  'Select Birthday': '选择生日日期',
  'All': '全部',
  'News': '新闻',
  'Games': '游戏',
  'Top': '置顶',
  'Popular Games': '热门游戏',
  'Supported Games': '支持的游戏',
  'Orders': '订单',
  'Paid': '已支付',
  'Comment': '评论',
  'Serving': '服务中',
  'Waiting': '等待接单',
  'Clear': '清除',
  'Delete': '删除',
  'UnTop': '取消置顶',
  'No Conversation': '没有会话',
  'Input your comment': '请输入评论',
  'Input your reasons for refusal': '请输入拒绝理由',
  'Reject Order': '拒绝订单',
  'COMMIT': '提交',
  'Refund': '退款',
  'Input your refund reason': '请输入退款理由',
  'Input your reject reason': '请输入拒绝理由',
  'Order Detail': '订单详情',
  'Service': '项目',
  'Status': '状态',
  'Score': '评分',
  'CANCEL': '取消',
  'Cancel Order': '取消订单',
  'Do you want to cancel this order?': '你是否确定取消订单？',
  'ACCEPT': '接受',
  'REJECT': '拒绝',
  'FINISHED': '完成',
  'REFUND': '退款',
  'Information': '订单信息',
  'Order Number': '订单编号',
  'Service Time': '订单时间',
  'Service Duration': '服务',
  'From': '下单人',
  'To': '接单人',
  'RejectReason': '拒绝理由',
  'Reason': '理由',
  'Comments': '评论',
  'Total': '总计',
  'Pay': '支付',
  'Add Block List': '添加黑名单',
  'Do you want to add this person to black list?': '是否要将此人添加到黑名单?',
  'Edit': '编辑',
  'Block': 'Block',
  'Online': '在线',
  'OffLine': '离线',
  'UnFollow': '取消关注',
  'Follow': '关注',
  'Network exception': '网络错误',
  'Message': '消息',
  'Follows': '关注',
  'Fans': '粉丝',
  'Services': '订单',
  'Album': '相册',
  'More': '更多',
  'Play Order': '陪玩订单',
  'Messages': '发消息',
  'Press again to exit': '再按一次退出应用',
  'Home': '首页',
  'Activities': '活动',
  'SideKick': '陪玩',
  'Shop': '商场',
  'Profile': '个人资料',
  'system': 'system',
  'System Notification': '系统消息',
  'system notification': '系统消息',
  'ticker': 'ticker',
  'Events': '活动',
  'Tournaments': '比赛',
  'Team Passcode': '邀请码',
  'Please remember your team passcode and send it to your team members.':
  '请记住您的团队密码并将其发送给您的团队成员.',
  'COPY': '复制',
  'JOIN': '加入',
  'Overview': '概况',
  'Rules': '规则',
  'Participants': '参与者',
  'Prizes': '奖励',
  'Choose a Store': '选择门店',
  'Congratulations': '恭喜你',
  'You have successfully signed up!': '您已成功注册!',
  'Select Location': '选择位置',
  'Location': '位置',
  'Please select location': '请选择位置',
  'Cup Sleeve': 'Cup Sleeve',
  'cup sleeve required': 'cup sleeve required',
  'confirm': '确认',
  'VIEW MY TEAM': '查看我的团队',
  'JOIN TEAM': '加入团队',
  'MAKE TEAM': '创建团队',
  'Make a team': '创建团队',
  'Join a team': '加入团队',
  'TEAM NAME': '团队名称',
  'input your team name': '请输入团队名称',
  'TEAM PASSCODE': '邀请码',
  'input your team passcode': '请输入邀请码',
  'PLAYING ROLE': '角色',
  'input your playing role': '请输入角色',
  'DISCORD TAG': 'DISCORD TAG',
  'input your discord tag': '输入Discord Tag',
  'LOCATION': '位置',
  'Copy': '复制',
  'The team passcode copied, you can send it to your team members.':
  '团队邀请码已复制，您可以发送给您的团队成员。',
  'Please input your team name': '请输入团队名称',
  'Please input correct team passcode': '请输入正确邀请码',
  'Please input your playing role': '请输入角色',
  'Please input your discord tag': '请输入Discord 标签',
  'Tips': '小贴士',
  'time': '时间',
  'Start Time': '开始时间',
  'Check In': '签到时间',
  'fee': '报名费',
  'Price': '价格',
  'day': '天',
  'Constraint': '限制条件',
  'Formation': '赛制',
  'game': '游戏',
  'Game': '游戏',
  'online': '线上',
  'Location&Participants': '门店&参与者',
  'GENERAL INFORMATION': '比赛介绍',
  'Ranking': '排行',
  'My Team': '我的队伍',
  'The team passcode has been copied to your clipboard': '复制成功',
  'Confirm': '确定',
  'Successfully Cancelled!': '取消成功！',
  'We will charge a deposit of £': '我们将从您的余额中扣除£',
  'from your balance for this sign up, Please make sure that you have enough balance.':
  '作为报名的预定金，请确保您的余额足够。',
  'Deposit Required': '定金',
  'Select Time': '选择时间',
  'Pin Required': '请输入支付密码',
  'Input your pin': '输入支付密码',
  'Forgotten?': '忘记密码？',
  'Please input your payment pin': '请输入支付密码',
  'Wrong payment pin': '支付密码错误',
  'New Version Available': '有新版本',
  'UPGRADE': '升级',
  'Upgrade already started': '升级已开始',
  'Permission required': '需要权限',
  'File access denied, please click the button below to change current setting.':
  '文件访问被拒绝，请点击下方按钮更改当前设置。',
  'Upgrade failed': '上传失败',
  'Time': '时间',
  'Quota': 'Quota',
  'Alipay': '支付宝',
  'Gold Coins': '金币',
  'Balance': '余额',
  'Amount': '金额',
  'Please select your billing address': '请选择您的账单地址',
  'Billing Address': '帐单地址',
  'Credit Card': '银行卡',
  'Server response error!': '服务错误!',
  'Checking payment result ...': '检测支付结果 ...',
  'Checking payment status ...': '检测支付状态 ...',
  'Payment Failed!': '支付失败！',
  'Payment Result': '支付结果',
  'Insufficient coin, Please recharge first!': '余额不足，请先充值!',
  'Payment Successful!': '支付成功！',
  'The payment result can not be confirmed, do you have finished it?':
  '无法确认付款结果，您是否已完成支付？',
  'The payment result still can not be confirmed, please contact our customer service.':
  '仍无法确认付款结果，请联系我们的客服。',
  'The payment has been canceled.': '支付被取消',
  'Warning': '提醒',
  'Are you sure to delete this account ?': '你确定要删除账户？',
  'Wallet Records': '钱包记录',
  'WithDraw': '提现',
  'Coin': '金币',
  'Diamonds': '钻石',
  'Total amount': '余额',
  'Enter withdrawal amount': '提现金额(钻石)',
  'Withdrawal amount': '提现金额(钻石)',
  'Min': '最少',
  'Withdrawal Account': '用户账户',
  'Only 4 bankcards allowed!': '最多绑定4张银行卡!',
  'Add Account': '添加账户',
  'Exchange To Coin': '充值金币',
  'Withdrawal instructions:': '提现和兑换指示:',
  'If you withdraw cash from us, you’ll be charged a handling fee of 3%.':
  '如果您从我们这里提现现金，将收取3%的手续费。',
  '6 Diamond for £1.': '6钻石对应1英镑.',
  'Withdrawals typically take three to five bank working days.':
  '提款通常需要三到五个银行工作日.',
  'Recharge': '充值金币',
  'Other recharge amount': '自定义金额',
  'Please enter an valid number greater than 1': '最少充值金额:£1',
  'Please Enter withdraw amount!': '请输入提现金额!',
  'Please enter an valid number greater than': '金额不能小于',
  'Please enter an valid number smaller than': '请输入金额小于',
  'Please Add withdraw account First!': '请先输入提现账户!',
  'All Services': '所有服务',
  'Accompany Certification': 'Accompany Certification',
  'The following items are required. To ensure your interests, please fill them out truthfully':
  '以下项目为必填项。为了保障您的权益，请如实填写。',
  'Reserve': '预定',
  'Please enter realName': '请输入真实名字',
  'Please enter idNumber': '请输入身份证号码',
  'Please enter phoneCon': 'Please enter phoneCon',
  'Please enter verification code': '请输入验证码',
  'Please upload ID card front photo': '请上传身份证正面照',
  'Please upload ID card back photo': '请上传身份证反面照',
  'Please select category': '请选择分类',
  'Please select service': '请选择服务',
  'Please enter be Good At': '请输入你的擅长领域',
  'Category': '分类',
  'Please select': '请选择',
  'Please check the network settings': '请检查网络设置',
  'Please select category first': '请先选择分类',
  'Select service': '选择服务',
  'Service Level': '服务等级',
  'Please select service first': '请先选择服务',
  'Please select service level': '请选择服务段位',
  'Select level': '选择等级',
  'Be good at': '擅长',
  'ID Photo': 'ID Photo',
  'Upload ID photo': '上传身份证照片',
  'Basic information': '基础资料',
  'Real name': '真实姓名',
  'ID number': 'ID number',
  'your phone number': '手机号',
  'verification code': '验证码',
  'Get code': 'Get code',
  'edit service': '编辑服务',
  'add service': '添加服务',
  'Please enter the price': '请输入价格',
  'The price cannot be less than the minimum value': '价格不能低于区间最小值',
  'The price cannot be greater than the maximum value': '价格不能高于区间最大值',
  'Please upload screenshot': '请上传游戏证明截图',
  'Submitted successfully': '提交成功',
  'Service detail': '游戏详情',
  'No service': '没有服务',
  'Select Service': '选择服务',
  'Level': '段位',
  'Price range': '价格区间',
  'Enable': '开启',
  'Screenshot': '截图',
  'Uploading failed, please try again later':
  '上传失败，请稍后重试',
  '￡10.99/30min': '￡10.99/30分钟',
  'Very Good': '非常好',
  'Leaving a message': '留言',
  'If you have any comments on this order, or want to say any questions and comments, you can mention them here':
  '如果您对此订单有任何意见，或者想要提出任何问题和评论，您可以在此处提出',
  'My earnings': '我的收入',
  'Order': '下单',
  'Received': '我收到的',
  'Provided': '我发起的',
  'No more': '没有更多',
  'Play Profile': '个人资料',
  'OK': '确定',
  'Please upload your avatar': '请上传头像',
  'Please enter user nickname': '请输入昵称',
  'The nick name cannot exceed 26 characters': '昵称不能超过26个汉字',
  'Please select language': '请选择语言',
  'Please enter your signature': '请输入个性签名',
  'Signature cannot exceed 255 characters': '前面不能超过255好字符',
  'Please upload your background image': '请上传背景图',
  'Please select your country': '请选择国籍',
  'Please upload your album': '请上传相册图片',
  'Background': '背景图',
  'Nickname': '昵称',
  'Gender': '性别',
  'Select Gender': '选择性别',
  'Language': '语言',
  'Please language': '语言',
  'No language to choose': '没有可选择的语言',
  'Select Language': '选择语言',
  'Signature': '个性签名',
  'Please enter Signature': '请输入个性签名',
  'Country': '国家',
  'State': '省',
  'City': '城市',
  'My Services': '我的技能',
  'Add Service': '添加服务',
  'reject': '拒绝',
  'under review': '审核中',
  'edit': '编辑',
  "I'm good at League of heroes and eternity. Please come to me~":
  "我擅长英雄联盟和永恒之轮。请找我玩~",
  'Accompanying materials': '附加材料',
  'Personal information': '个人信息',
  'Good games': 'Good games',
  'LEAGUE OF LEGENDS': 'LEAGUE OF LEGENDS',
  'King 120star': 'King 120star',
  'Place an order': 'Place an order',
  'Play with': '一块玩',
  'Game support': '游戏支持',
  'Recharge successfully': '充值成功！',
  'Sickkick order has been placed, please wait for Sickkicker\nOr contact can contact Sickkicker':
  '已下单玩游戏，请等待一起游戏\n或联系Play with进行沟通',
  'Sent Message': 'Sent Message',
  'No more data': '没有更多数据',
  'Default': 'Default',
  // 'My Address': '我的地址',
  'NEW ADDRESS': '添加地址',
  'Please add your shipping address': '请输入收货地址',
  'Address': '地址',
  'Contact Info': '联系信息',
  'Input your first name': '请输入姓氏',
  'Input your last name': '请输入名字',
  'Email': 'Email',
  'Input your email address': '输入你的邮箱地址',
  'Phone': '电话',
  'Input your mobile phone number': '输入你的电话号码',
  'Shipping Address': '邮寄地址',
  'Address Line 1': 'Address Line 1',
  'Input your detailed address': '输入您的详细地址',
  'Address Line 2 (Optional)': 'Address Line 2 (可选)',
  'Post Code': '邮编',
  'Input your post code': '请输入邮编',
  'Input your city': '请输入城市',
  'Use this as default': '设为默认',
  'Please input a first name': '请输入姓氏',
  'Please input a last name': '请输入名字',
  'Please input a email': '请输入邮箱',
  'Please input a phone number': '请输入电话号码',
  'Please input a detail address': '请输入详细地址',
  'Please input a post code': '请输入邮编',
  'Please input your city': '请输入城市',
  'Unknown': '未知',
  'Unfollow': '取消关注',
  'Balance Records': '余额记录',
  'Top Up': '充值',
  'Other Top Up Amount': '自定义金额',
  'Top Up Account': '充值账户',
  'mins': '分钟',
  //分钟缩写
  'Credits Balance': '账户余额',
  'Free Gaming Time': '免费时长',
  'Bind bank card': '绑定银行卡',
  'In order to ensure normal bank card signing, you need to collect your bank card information to ensure privacy and security throughout the process. Please feel free to use':
  '为了确保正常的银行卡签约，您需要收集您的银行卡信息，以确保整个过程的隐私和安全。请放心使用',
  'Bank card information': '添加银行卡',
  "By signing up to Wanyoo\'s tournament, you agree to \n our": "我已阅读并同意 ",
  'Tournament RulesPrivacy': 'Tournament RulesPrivacy',
  'Cookies Policy': 'Cookies Policy',
  'Next': '下一步',
  'SWIFT code': 'SWIFT code',
  'Please enter the 6-digit format xx-xx-xx': '请输入6位数字',
  'Recipient Bank Name': '收款银行名称',
  'please input': '请输入',
  'Recipient Bank Account Number': '帐号',
  'Name on account': '帐户名称',
  'Cancel Confirm': '取消确认',
  'Do you confirm to cancel this booking?': '你想要取消这个预定吗?',
  'Bookings': '我的预定',
  'MAKE A NEW BOOKING': '预定',
  'Cancel Booking': '取消',
  'Book A Room': '预定房间',
  'BOOK': '预定',
  'Store': '门店',
  'Select One Store': '选择门店',
  'Area': '区域',
  'Select One Area': '区域',
  'What Time': '到店时间',
  'How Long': '预定时长',
  'Contact Phone': '联系电话',
  '* Any Events / BootCamp / Birthday booking requirements please contact our customer service directly.':
  '* 任何活动/训练营/生日预订需求，请直接联系我们的客服。',
  '* We require at least 4 people to attend bookings for Battle Rooms or Squad Rooms, and a minimum of 2 people for Duo Rooms.':
  '* 我们要求至少4人参加战斗房间或小队房间的预订，至少2人参加双人房间的预订。',
  '* If you arrive more than half an hour after your booking time, your reservation will be invalidated.':
  '* 如果您在预订时间之后半小时到达，您的预订将无效。',
  "* Please note that if you don't meet the above criteria, the deposit will not be refundable.":
  "* 请注意，如果您不符合上述条件，押金将不可退还。",
  'Select Store': '选择门店',
  'Select Area': '选择区域',
  'closed at this time, please choose another time':
  '此时间段关闭，请选择其他时间。',
  'not open at this time, please choose another time.':
  '此时间段未开放，请选择其他时间。',
  '1 hour': '1 小时',
  'hours': ' 小时',
  'Please select store location': '请选择门店位置',
  'Please select an area': '请选择一个区域',
  'Please select how long': '请选择多长时间',
  'Please select what time': '请选择时间',
  'Please input your phone': '请输入您的电话号码',
  'Congratulations!': '恭喜!',
  'Your room is reserved. A confirmation email will send to you shortly.':
  '您的房间已预订。确认电子邮件将很快发送给您.',
  'We will charge a deposit of': '我们将收取押金',
  'To book this area, a booking fee is required. Please ensure that you have sufficient balance.':
  '从您的余额中预订此区域，请确保您有足够的余额。',
  'My Bill': '我的账单',
  'Gaming': '上网',
  'Product': '商品',
  'Expire Date': '过期日期',
  'off': 'off', //${model.discount} % ${'off'.tr 语境有可能是折扣
  'My Vouchers': '我的优惠券',
  'ADD': '添加',
  'Voucher Added': '添加优惠券',
  'Input your voucher code': '请输入优惠券码',
  'Please input your voucher code': '请输入优惠券码',
  'Expire date': '过期日期',
  'Developer': '开发者',
  'Push Token': 'Push Token',
  'The push token has been copied to your clipboard':
  '已复制到剪贴板',
  'Environment': '环境',
  'Select your birthday': '选择日期',
  'Edit photo': '编辑图片',
  'Done': '完成',
  'Birthday': '生日',
  'Registered Date': '注册日期',
  "Deleting your account will remove your profile and all of your content from SideQuest. Delete account means you won't be able to get any of your data back. All your SideQuest account data will be deleted. If you experienced an issue with your account and need help, please contact us so we can assist you.This action cannot be UNDONE. Are you sure you need to DELETE ACCOUNT?":
  "删除账户将会删除您在SideQuest上的个人资料和所有内容。删除账户意味着您将无法恢复任何数据。您的SideQuest账户数据将被永久删除。如果您在账户上遇到问题并需要帮助，请与我们联系，我们将协助您。此操作无法撤销。您确定要删除账户吗？",
  'Delete Account': '删除账户',
  'Medal': 'Medal',
  'LEVEL': '等级',
  'Top level': '最高等级',
  'Current level': '当前等级',
  'SideKick Order': '陪玩订单',
  'Monthly recharge': '月充值',
  'Monthly consumption': '月消费',
  'Next Level': '下一等级',
  'Notifications': '通知',
  'Repay': 'Repay', //

  'Pending': '待支付',
  'Delivered': '已完成',
  'About us': '关于我们',
  "SideQuest is dedicated to being the social hub for gamers. SideQuest is more than just entertainment and games— it's about fresh experiences and community.":
  "SideQuest is dedicated to being the social hub for gamers. SideQuest is more than just entertainment and games— it's about fresh experiences and community.",
  "In the modern era of rapid technological and virtual development, we need to find a place for all gamers and the new generation to connect to the real world from the virtual. SideQuest Gamers Hub, on the other hand, starts with the gamer's experience and prioritises everything with the gamer's needs in mind, offering the best services, popular games and diverse social experiences.":
  "In the modern era of rapid technological and virtual development, we need to find a place for all gamers and the new generation to connect to the real world from the virtual.SideQuest Gamers Hub, on the other hand, starts with the gamer's experience and prioritises everything with the gamer's needs in mind, offering the best services, popular games and diverse social experiences.",
  "We use the central control technology by operating the computer from the server room and connecting the perfect ecology of all systems through the mobile phone APP. In this way, players can enjoy everything that is so simple and convenient. Our offline store is divided into 5 areas: public area, duo room, private room, battle room and viewing area. Whether you're alone, as a couple, in a team, or just want to watch the game live with friends, at SideQuest you'll always feel part of something bigger.":
  "We use the central control technology by operating the computer from the server room and connecting the perfect ecology of all systems through the mobile phone APP. In this way, players can enjoy everything that is so simple and convenient. Our offline store is divided into 5 areas: public area, duo room, private room, battle room and viewing area. Whether you're alone, as a couple, in a team, or just want to watch the game live with friends, at SideQuest you'll always feel part of something bigger.",
  "SideQuest is a metamorphosis of the original Wanyoo UK team. The game shouldn't be about, colour, language, nationality. The world of gamers is diverse, rich and colourful. We open our doors to all gamers with a sense of reverence for the world of gaming.":
  "SideQuest is a metamorphosis of the original Wanyoo UK team. The game shouldn't be about, colour, language, nationality. The world of gamers is diverse, rich and colourful. We open our doors to all gamers with a sense of reverence for the world of gaming.",
  'Account Password': '账户密码',
  'Old Password': '旧密码',
  'Old Pin': '旧支付密码',
  'Input your old password': '请输入旧账户密码',
  'Input your old pin': '请输入旧支付密码',
  'New Password': '新密码',
  'New Pin': '新支付密码',
  'Input your new password': '请输入新账户密码',
  'Input your new pin': '请输入新支付密码',
  'Password can not less than 6 characters': '支付密码不能少于6位',
  'Settings': '设置',
  'About Us': '关于我们',
  'Cancel Subscription': '取消订阅',
  'Version': '版本',
  'SIGN OUT': '退出登录',
  'You are using the latest version': '当前已经是最新版本',
  'Up': 'Up',
  'VIP Subscription': '订阅VIP',
  'VIP Upgrade Terms & Conditions': 'VIP升级条款和条件',
  'Acceptance of Terms of use': '接受使用条款',
  'These terms of use which include our privacy policy (link here to privacy policy) and cookie policy (link to cookie policy).':
  '这些使用条款包括我们的隐私政策（链接到隐私政策）和Cookie政策（链接到Cookie政策）。',
  'Changes to terms of use': '使用条款的变更',
  "SideQuest from time to time may change these terms of use including the privacy policy and cookie policy, such revisions shall be effective immediately provided however, for existing members, such revision shall, unless otherwise stated, be effective 30 days after posting. We will endeavour to post prior versions (including marked changes) of the Terms of Use, if any, for the preceding 12-month period.":
  "SideQuest from time to time may change these terms of use including the privacy policy and cookie policy, such revisions shall be effective immediately provided however, for existing members, such revision shall, unless otherwise stated, be effective 30 days after posting. We will endeavour to post prior versions (including marked changes) of the Terms of Use, if any, for the preceding 12-month period.",
  "Personally, identifying information is subject to our Privacy Policy https://sidequesthub.com/privacy, the terms of which are incorporated herein. Please review our Privacy Policy to understand our practices.":
  "Personally, identifying information is subject to our Privacy Policy https://sidequesthub.com/privacy, the terms of which are incorporated herein. Please review our Privacy Policy to understand our practices.",
  'Privacy': 'Privacy',
  "We offer subscription plans for a fee that provide benefits to a comprehensive experience and additional SideQuest exclusive offers. Your payment to us will automatically renew and continue until terminated. All payments are non-refundable. You must cancel your subscription one month in advance to avoid paying the subscription fee for the next billing period. We reserve the right to modify, terminate, or otherwise amend the subscription plans we offer from time to time. If you have purchased a Subscription, we will give you advance notice of significant changes to your plan.":
  "We offer subscription plans for a fee that provide benefits to a comprehensive experience and additional SideQuest exclusive offers. Your payment to us will automatically renew and continue until terminated. All payments are non-refundable. You must cancel your subscription one month in advance to avoid paying the subscription fee for the next billing period. We reserve the right to modify, terminate, or otherwise amend the subscription plans we offer from time to time. If you have purchased a Subscription, we will give you advance notice of significant changes to your plan.",
  'Payment Method': '支付方式',
  "Unless otherwise indicated, you are required to provide a credit card or other payment method that we, accept to pay the applicable fee for a Subscription. We will charge the payment method you’ve chosen a subscription fee plus any applicable taxes on a recurring basis corresponding to the term of your Subscription. You are solely responsible for all fees charged to the payment method you’ve chosen. We reserve the right to cancel your Subscription if we are unable to successfully charge your payment method.":
  "Unless otherwise indicated, you are required to provide a credit card or other payment method that we, accept to pay the applicable fee for a Subscription. We will charge the payment method you’ve chosen a subscription fee plus any applicable taxes on a recurring basis corresponding to the term of your Subscription. You are solely responsible for all fees charged to the payment method you’ve chosen. We reserve the right to cancel your Subscription if we are unable to successfully charge your payment method.",
  'Billing Period': 'Billing Period',
  "Recurring Billing. By starting your SideQuest membership, you authorize us to charge you a monthly membership fee at the then current rate, and any other charges you may incur in connection with your use of the SideQuest service to your Payment Method. You acknowledge that the amount billed each month may vary from month to month for reasons that may include differing amounts due to promotional offers and/or changing or adding a plan, and you authorize us to charge your Payment Method for such varying amounts, which may be billed monthly in one or more charges.":
  "Recurring Billing. By starting your SideQuest membership, you authorize us to charge you a monthly membership fee at the then current rate, and any other charges you may incur in connection with your use of the SideQuest service to your Payment Method. You acknowledge that the amount billed each month may vary from month to month for reasons that may include differing amounts due to promotional offers and/or changing or adding a plan, and you authorize us to charge your Payment Method for such varying amounts, which may be billed monthly in one or more charges.",
  'Billing Cycle': 'Billing Cycle',
  "The membership fee for our service will be billed at the beginning of the paying portion of your membership and each month thereafter unless and until you cancel your membership. We automatically bill your Payment Method each month on the calendar day corresponding to the commencement of your paying membership. Membership fees are fully earned upon payment. We reserve the right to change the timing of our billing, in particular, as indicated below, if your Payment Method has not successfully settled. In the event your paying membership began on a day not contained in a given month, we may bill your Payment Method on a day in the applicable month or such other day as we deem appropriate. For example, if you started your SideQuest membership or became a paying member on January 31st, your next payment date is likely to be February 28th, and your Payment Method would be billed on that date.":
  "The membership fee for our service will be billed at the beginning of the paying portion of your membership and each month thereafter unless and until you cancel your membership. We automatically bill your Payment Method each month on the calendar day corresponding to the commencement of your paying membership. Membership fees are fully earned upon payment. We reserve the right to change the timing of our billing, in particular, as indicated below, if your Payment Method has not successfully settled. In the event your paying membership began on a day not contained in a given month, we may bill your Payment Method on a day in the applicable month or such other day as we deem appropriate. For example, if you started your SideQuest membership or became a paying member on January 31st, your next payment date is likely to be February 28th, and your Payment Method would be billed on that date.",
  'Price Changes': 'Price Changes',
  "We may change our subscription plans and the price of our service from time to time; however, any price changes or changes to your subscription plans will apply no earlier than 30 days following notice to you.":
  "We may change our subscription plans and the price of our service from time to time; however, any price changes or changes to your subscription plans will apply no earlier than 30 days following notice to you.",
  'No Refunds': 'No Refunds',
  'PAYMENTS ARE NONREFUNDABLE AND THERE ARE NO REFUNDS OR CREDITS FOR PARTIALLY USED PERIODS. Following any cancellation, however, you will continue to have access to the service through the end of your current billing period. At any time, and for any reason, we may provide a refund, discount, or other consideration to some or all of our members ("credits"). The amount and form of such credits, and the decision to provide them, are at our sole and absolute discretion. The provision of credits in one instance does not entitle you to credits in the future for similar instances, nor does it obligate us to provide credits in the future, under any circumstance.':
  'PAYMENTS ARE NONREFUNDABLE AND THERE ARE NO REFUNDS OR CREDITS FOR PARTIALLY USED PERIODS. Following any cancellation, however, you will continue to have access to the service through the end of your current billing period. At any time, and for any reason, we may provide a refund, discount, or other consideration to some or all of our members ("credits"). The amount and form of such credits, and the decision to provide them, are at our sole and absolute discretion. The provision of credits in one instance does not entitle you to credits in the future for similar instances, nor does it obligate us to provide credits in the future, under any circumstance.',
  'Cancellations': 'Cancellations',
  "In order to cancel your subscription, you may cancel through the app or by contacting us at  support@sidequestmeta.com, you will continue to have access to the SideQuest service through to the end of your monthly billing period. WE DO NOT PROVIDE REFUNDS OR CREDITS FOR ANY PARTIAL-MONTH MEMBERSHIP PERIODS":
  "In order to cancel your subscription, you may cancel through the app or by contacting us at  support@sidequestmeta.com, you will continue to have access to the SideQuest service through to the end of your monthly billing period. WE DO NOT PROVIDE REFUNDS OR CREDITS FOR ANY PARTIAL-MONTH MEMBERSHIP PERIODS",
  'Passwords and Account Access': 'Passwords and Account Access',
  'You must be at least 16 years of age to become a member of the SideQuest VIP service. Those under the required age may only have access to the base membership.':
  'You must be at least 16 years of age to become a member of the SideQuest VIP service. Those under the required age may only have access to the base membership.',
  'SideQuest Service': 'SideQuest Service',
  'The SideQuest service and the SideQuest equipment used during the service is for the personal use of the account holder. During your SideQuest membership, we grant you a limited, non-exclusive, non-transferable right to access the SideQuest service and SideQuest equipment. Except for the foregoing, no right, title or interest shall be transferred to you.':
  'The SideQuest service and the SideQuest equipment used during the service is for the personal use of the account holder. During your SideQuest membership, we grant you a limited, non-exclusive, non-transferable right to access the SideQuest service and SideQuest equipment. Except for the foregoing, no right, title or interest shall be transferred to you.',
  'You may access SideQuest equipment in our stores, our VIP service allows for varying benefits and access to equipment depending on the VIP service plan that you are currently subscribed to as a member, including access to available beverages and depending on circumstances member nights which include industry talks, events, and challenges.':
  'You may access SideQuest equipment in our stores, our VIP service allows for varying benefits and access to equipment depending on the VIP service plan that you are currently subscribed to as a member, including access to available beverages and depending on circumstances member nights which include industry talks, events, and challenges.',
  'Your SideQuest monthly benefits reset on a monthly basis and do not accrue those benefits are hours on SideQuest equipment and bubble teas.':
  'Your SideQuest monthly benefits reset on a monthly basis and do not accrue those benefits are hours on SideQuest equipment and bubble teas.',
  'You agree not to archive, reproduce, distribute, modify, display, perform, publish, license, create derivative works from, offer for sale, or use (except as explicitly authorized in these Terms of Use) content and information contained on or obtained from or through the SideQuest service or equipment. You also agree not to: use any robot, spider, scraper or other automated means to access the SideQuest service or equipment; decompile, reverse engineer or disassemble any software or other products or processes accessible through the SideQuest service or equipment; insert any code or product or manipulate the content of the SideQuest service in any way; or use any data mining, data gathering or extraction method. In addition, you agree not to upload, post, e-mail or otherwise send or transmit any material designed to interrupt, destroy or limit the functionality of any computer software or hardware or telecommunications equipment associated with the SideQuest service, including any software viruses or any other computer code, files or programs. We may terminate or restrict your use of our service if you violate these Terms of Use or are engaged in illegal or fraudulent use of the service.':
  'You agree not to archive, reproduce, distribute, modify, display, perform, publish, license, create derivative works from, offer for sale, or use (except as explicitly authorized in these Terms of Use) content and information contained on or obtained from or through the SideQuest service or equipment. You also agree not to: use any robot, spider, scraper or other automated means to access the SideQuest service or equipment; decompile, reverse engineer or disassemble any software or other products or processes accessible through the SideQuest service or equipment; insert any code or product or manipulate the content of the SideQuest service in any way; or use any data mining, data gathering or extraction method. In addition, you agree not to upload, post, e-mail or otherwise send or transmit any material designed to interrupt, destroy or limit the functionality of any computer software or hardware or telecommunications equipment associated with the SideQuest service, including any software viruses or any other computer code, files or programs. We may terminate or restrict your use of our service if you violate these Terms of Use or are engaged in illegal or fraudulent use of the service.',
  'The member who created the SideQuest account and whose Payment Method is charged (the "Account Owner") is responsible for any activity that occurs through the SideQuest account. To maintain control over the account and to prevent anyone from accessing the account (which would include information on viewing history for the account), the Account Owner should maintain control over the SideQuest ready devices that are used to access the service and not reveal the password or details of the Payment Method associated with the account to anyone. You are responsible for updating and maintaining the accuracy of the information you provide to us relating to your account. We can terminate your account or place your account on hold in order to protect you or SideQuest from identity theft or other fraudulent activity.':
  'The member who created the SideQuest account and whose Payment Method is charged (the "Account Owner") is responsible for any activity that occurs through the SideQuest account. To maintain control over the account and to prevent anyone from accessing the account (which would include information on viewing history for the account), the Account Owner should maintain control over the SideQuest ready devices that are used to access the service and not reveal the password or details of the Payment Method associated with the account to anyone. You are responsible for updating and maintaining the accuracy of the information you provide to us relating to your account. We can terminate your account or place your account on hold in order to protect you or SideQuest from identity theft or other fraudulent activity.',
  'Invite Only': 'Invite Only',
  'Subscribed': '订阅',
  'VIP Benefits': 'VIP权益',
  'Subscription members must be at least 16 years old.':
  '订阅会员必须至少16岁。',
  'Mins': '分钟',
  'Remaining': '剩余',
  'Vouchers': '优惠券',
  'Free Time': '免费时长',
  'Play Function': 'Play Function',
  'Wallet': '钱包',
  'Store Section': 'Store Section',
  'My Balance': '我的账户',
  'Scan QR code': '扫描二维码',
  'Search anything you want to buy': '搜索你想要买的',
  'Shopping Cart': '购买',
  'Subtotal': '小计',
  'VAT': '税',
  'Delivery': '运费',
  'ADD TO CART': '加入购物车',
  'price': '价格',
  'PRODUCT DETAILS': '商品详情',
  'Specification': 'Specification',
  'No numeric keypad': 'No numeric keypad',
  'Numeric keypad': 'Numeric keypad',
  'Wireless keypad': 'Wireless keypad',
  'Combination': 'Combination',
  'Quantity': '数量',
  'RECOMMEND': '推荐',
  'Cart': '购买',
  'Stores': '门店',
  'Withdrawal': '提现',
  'Are you sure to delete this?': '确定要删除吗?',
  'Service Name': '服务项目',
  'Rank': '段位',
  '暂无': '暂无',
  'the service price not in the price range': '服务价格不在价格区间内',
  'Play': '下单',
  'SUBTOTAL': '小计',
  'Service Tax': '服务税',
  'Please enter discount password here': '请在此处输入折扣密码',
  'please enter voucher code': '请输入优惠券码',
  'Submit': '提交',
  'TOTAL': '总计',
  'Coupon deduction': '抵扣',
  'Discount': '折扣',
  'Available': '可用',
  'Overdue': 'Overdue',
  'Rejected': '拒绝',
  'Completed': '已完成',
  'Canceled': '已取消',
  'Accepted': '已接单',
  'Refund Pending': '等待退款',
  'Refund Rejected': '拒绝退款',
  'Refund Accepted': '同意退款',
  'Refund Dispute': '客服介入',
  'Seller Payment Terms': '陪玩支付协议',
  'Diamond': '钻石',
  'Credits': '余额',
  'Disabled': '已关闭',
  'Actual payment': '实际支付',
  'Voucher Unavailable': '优惠券不可用',
  'Input your payment pin': '支付密码',
  'Add Voucher': '添加优惠券',
  '* These Coins are only used for SideKick.': '* 金币仅限陪玩服务使用。',
  '* These Credits are only used for SideQuest Hub.': '* 余额仅限支线电竞线下门店使用。',
  'These Coins are only used for SideKick.': '金币仅限陪玩服务使用。',
  'These Credits are only used for SideQuest Hub.': '余额仅限支线电竞线下门店使用。',
  'SideKick Section': 'SideKick Section',
  'Profit': '收益',
  'America': 'America',
  'Andorra': 'Andorra',
  'Austria': 'Austria',
  'Belarus': 'Belarus',
  'Belgium': 'Belgium',
  'Bosnia and Herzegovina': 'Bosnia and Herzegovina',
  'Bulgaria': 'Bulgaria',
  'China': '中国',
  'Cyprus': 'Cyprus',
  'Denmark': 'Denmark',
  'Estonia': 'Estonia',
  'Faroe Islands': 'Faroe Islands',
  'Finland': 'Finland',
  'France': '法国',
  'Germany': 'Germany',
  'Greece': 'Greece',
  'Hungary': 'Hungary',
  'Iceland': 'Iceland',
  'Ireland': 'Ireland',
  'Italy': 'Italy',
  'Latvia': 'Latvia',
  'Liechtenstein': 'Liechtenstein',
  'Lithuania': 'Lithuania',
  'Luxembourg': 'Luxembourg',
  'Macedonia': 'Macedonia',
  'Malta': 'Malta',
  'Moldova': 'Moldova',
  'Monaco': 'Monaco',
  'Norway': 'Norway',
  'Poland': 'Poland',
  'Portugal': 'Portugal',
  'Romania': 'Romania',
  'Russia': 'Russia',
  'San Marino': 'San Marino',
  'Serbia': 'Serbia',
  'Slovakia': 'Slovakia',
  'Slovenia': 'Slovenia',
  'Spain': 'Spain',
  'Sweden': 'Sweden',
  'Switzerland': 'Switzerland',
  'United Kingdom': '英国',
  'zh': '中文',
  'en': '英文',
  'Rating': '评分',
  'Services ': '服务 ',
  'Nonconforming': '未知',
  'Refund Reject Reason': '拒绝退款原因',
  'Refund Reason': '退款理由',
  'Experience': '经验',
  'Online Time': '在线时长',
  "Account": "账户",
  "Recommend": "推荐",
  "Non-binary": "未知",
  "Recipient Bank Address (Optional)": "收款行地址(可选)",
  "Recipient Bank Address": "收款行地址",
  "Recipient’s Bank Account Name": "收款账户名称",
  "Recipient’s Bank Country": "收款银行国家",
  "Refunded": "已退款",
  "Lack of diamonds": "钻石不足",
  "You should read and agree to our seller payment terms first.":
  "请先阅读并同意我们的陪玩支付协议。",
  "RETRIEVE": "找回",
  "Recipient’s Bank Bill Address": "账单地址",
  "These coins are only used for SideKick": "金币仅限陪玩服务使用",
  "WITHDRAW": "提现",
  'These credits are only used for SideQuest Hub': '余额仅限支线电竞线下门店使用',
  'View/Add your vouchers': '查看/添加优惠券',
  "VIEW": "查看",
  "TOP UP": "充值",
  "SideKick Coin": "金币",
  "6 Diamonds for £1": "6钻石兑换1英镑",
  "Please Enter paypal account!": "请输入Paypal账户!",
  "Input nickname,UK account or email": "请输入昵称,UK账户或者Email",
  "Up to Four": "最多选择四个",
  "Please input service name": "请输入服务名字",
  "Order Timeline": "订单状态",
  "Posts": "动态",
  "Dashboard": "主页",
  "Post": "动态",
  "Social": "广场",
  "Activity": "活动",
  "Subscriptions": "订阅",
  "Trophies": "勋章",
  "Sidekick": "陪玩",
  "Followers": "粉丝",
  " Up": "收起",
  "Benefits": "权益",
  "My Wallet": "我的钱包",
  "Remaining game time: ": "剩余免费时长: ",
  "Style": "风格",
  "Server": "服务器",
  "Position": "位置",
  "My Activities": "我的活动",
  "Method of receipt": "收款方式",
  "please input your account": "请输入账户",
  "+ Follow": "关注",
  "Visitors": "访客",
  "Request a refund": "申请退款",
  "Reason for refund": "退款原因",
  "If the player agrees, the coin will be refunded to your balance. However, if the player rejects your refund, you can seek assistance from the official support team.":
  "陪玩同意退款后金币降自动返回你的账户中，你也可以联系客服获取帮助",
  "Justification": "申诉理由",
  "Customer Service": "官方客服",
  "Refund Amount": "退款金额",
  "Select a reason": "请选择原因",
  "We suggest that you supplement the recording materials": "请上传录音",
  "My Orders": "我的订单",
  "Others": "其他",
  "Order ID": "订单号",
  "Order Time": "下单时间",
  "Performance": "专业度",
  "Responsive": "响应速度",
  "Enjoyment": "娱乐程度",
  "Friendless": "友好程度",
  "Order details": "订单详情",
  "User Rating": "用户评分",
  "My Bookings": "我的预定",
  "Ablum": "相册",
  "My Subscription": "我的订阅",
  "Support": "帮助",
  "Legal": "法规",
  "Terms of use": "使用条款",
  "Help Centre": "帮助中心",
  "Give us feedback": "反馈",
  'Price Range': '价格区间',
  "Post photos(Optional)": "图片(可选)",
  "Add content": "请输入内容",
  "Post content": "发布动态",
  "Post Detail": "动态详情",
  "reply ": "回复 ",
  "reply:": "回复:",
  "Please enter comment!": "输入内容!",
  "The min price is 1": "最小价格为1金币",
  "The max price cannot be lower than the min price": "最高价格不能低于最低价格",
  "Please enter the max price": "请输入最高价格",
  "Please enter the min price": "请输入最低价格",
  "Sidekick Types ": "自定义选项 ",
  "(Optional)": "(可选)",
  "Additional Requests ": "其他需求 ",
  "Select Category": "选择类别",
  "Select Game": "选择游戏",
  "Send": "发送",
  "Server, Ranking, Roles etc.": "等级，角色，服务器，位置等",
  "Service Type": "服务类型",
  "Select Unit": "选择服务类型",
  "Sidekick Match": "广播下单",
  "Upload success": "上传成功",
  "Record": "录制",
  "From File": "上传文件",
  "Voice Record": "录音",
  "Select type": "选择类型",
  "Press and hold to record": "长按录音",
  "seconds ": "秒 ",
  "Record for at least ": "至少录制 ",
  "Introduce yourself in one": "用一句话介绍你自己",
  "sentence!": "",
  "Cancel": "取消",
  "Set as background": "设为背景图",
  "Badge": "徽章",
  "Services  ": "技能",
  "My Requirements": "我的要求",
  "SideKick Match": "广播下单",
  "unit": "单位",
  "Stop Matching": "取消订单",
  "Waiting for ": "等待时长",
  "E-Player Introduction": "自我介绍",
  "Click to edit avatar": "点击编辑头像",
  "only 9 pictures are allowed": "最多上传9张图片",
  "Are you sure confirm the order without any comments?": "你还没有输入评价，确认提交吗？",
  "Date of birth": "生日",
  "Select your date of birth": "请选择出生日期",
  "Please select your date of birth": "请选择出生日期",
  "Please select your gender": "请选择性别",
  "13+ only": "年满13岁",
  "Are you sure to delete this post?": "确定删除该动态?",
  "Replied": "我的回复",
  "Praised": "我的收藏",
  "My Posts": "我的动态",
  "Add More Service Types": "添加更多服务类型",
  "Please add service type": "至少添加一项服务内容",
  "Previous": "上一步",
  "Voice": "录音",
  "+ Add Voice": "+ 语音介绍",
  "Ongoing": "进行中",
  "Service Intro": "游戏介绍",
  "Please input service intro": "请添加游戏介绍",
  "This picture will be displayed in your service profile":
  "图片将作为封面显示在你的游戏资料页上",
  "Please record your voice, which will be displayed on your service profile":
  "语音将显示在你的游戏资料页上",
  "Please upload a picture as the service cover image": "请上传游戏封面图片",
  "Service type already exist": "服务类型已经存在",
  "Name": "名称",
  "ServiceType": "服务类型",
  "Insufficient Diamonds": "钻石不足",
  "WithDrawals": "提现",
  "Cancelled": "已取消",
  "Subscription Cancelled": "取消订阅",
  "Commented": "我回复的",
  "Liked": "我喜欢的",
  "Info": "主页",
  "Select Type": "选择类型",
  "Gift List": "礼物",
  "Please select rank": "请选择段位",
  "I already have an account": "登录",
  "Get Started": "注册",
  "Save": "保存",
  "Phone number": "电话号码",
  "Personal": "个人资料",
  "Followings": "关注",
  "Intimacy": "亲密度",
  "feedback": "反馈",
  "Styles": "风格",
  " Reviews": "评论",
  "Platforms": "平台",
  "Platform": "平台",
  "Policies": "政策",
  "Gift": "礼物",
  "Photo": "图片",
  "Service Types": "服务类型",
  "ServiceType_": "服务类型_",
  "Voice Recording": "录音",
  "Cover": "封面",
  "Pay Confirm": "支付",
  "NickName": "昵称",
  "My Followers": "我的粉丝",
  "My Followings": "我的关注",
  "My Visitors": "访问者",
  "1st Order Discount": "首单折扣",
  "Free Y": "折扣 Y",
  "Buy X": "购买 X",
  "Promotion": "优惠",
  "Choose a promotion": "选择优惠",
  "Promotion Setting": "优惠设置",
  "Setting Promotion": "优惠设置",
  "Buy X Get Y Free": "买X送Y",
  "Select currency": "选择货币",
  "Currency": "货币",
  "My Diamond": "我的钻石",
  "My Coin": "我的金币",
  "Service Charge": "提现手续费",
  "Rate": "提现费率",
  "Business hours": "营业时间",
  "Configuration list": "上网价格",
  "How long": "时长",
  "Select Players": "人数",
  "Room": "房间",
  "Select Room": "选择房间",
  "Players": "人数",
  "Book Now": "预定",
  "Reservation information": "预订信息",
  "Telephone": "电话",
  "Create Group": "创建群聊",
  "Group Name": "群名称",
  "Enter group name": "请输入群名称",
  "Group introduction": "群介绍",
  '清空聊天消息': 'Clear Chat Messages',
  '群成员': 'Group Members',
  '修改群名称': 'Modify Group Name',
  '群主、管理员': 'Group Owner, Administrators',
  '群公告': 'Group Announcement',
  '暂无群公告': 'No Group Announcement',
  '设置管理员': 'Set Administrators',
  '群管理': 'Group Management',
  '全员禁言': 'Mute All Members',
  '全员禁言开启后，只允许群主和管理员发言。': 'After enabling mute all members, only the group owner and administrators are allowed to speak.',
  '全员禁言中': 'Mute All Members',
  '添加需要禁言的群成员': 'Add group members to be muted',
  '置顶聊天': 'Pin Chat',
  '消息免打扰': 'Disable Message Notifications',
  '我的群昵称': 'My Group Nickname',
  '修改我的群昵称': 'Modify My Group Nickname',
  '仅限中文、字母、数字和下划线，2-20个字': 'Only Chinese, letters, numbers, and underscores, 2-20 characters',
  '清空聊天记录': 'Clear Chat History',
  '转让群主': 'Transfer Group Ownership',
  '解散该群': 'Dissolve Group',
  '解散后不会接收到此群聊消息': 'You will no longer receive messages from this group after dissolution.',
  "Quest":"任务",
  "Leaderboard":"排行榜",
  "Sidekicker":"陪玩榜",
  "Side Buddy":"消费榜",
  "Friendship":"亲密度",
  "Invite":"邀请",
  "Platform fee":"平台手续费率",
  "Gift Order":"礼物订单",
  "SideKicker Introduction":"自我介绍",
  "Month":"月榜",
  "Week":"周榜",
  "Day":"日榜",
  "Consumption record":"游戏记录",
  "Simple and quick signup, no review required.":"",
  "Lower fees, higher order acceptance rate, higher order prices. Requires 2-3 working days for approval.":"更低手续费，更高的价格，更高的点单率。审批时间2-3天。",
  "Entry Fee":"报名费",
  "Register via below link":"点击跳转正式报名地址：(需要自行组队找队友)",
  "Battlefly join code:":"您的报名验证码:",
  'Please enter an integer from 5 to 500':'请输入5-500的整数',
  "Custom amount":"自定义充值",
  "Link your game account to the SideQuest account":"链接你的游戏账户到SideQuest",
  "Link Riot account":"添加Riot账号",
  "Add game account":"添加游戏账户"
};
