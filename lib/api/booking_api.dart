...
class BookingApi {
  static Future<List<BookingModel>> list() async {
    var response = await http.get('/app/booking/list', queryParameters: ({}));
  ...
  static Future<List<BookingSelectModel>> listStores() async {
    ...
    var response =
        await http.get('/app/booking/211/stores', queryParameters: ({}));
  ...
  static Future<void> reserve(BookingModel model) async {
    ...
    await http.post('/app/booking/reserve', data: formData);
  }

  static Future<void> cancel(int id) async {
    await http.get('/app/booking/cancel', queryParameters: ({"bookid": id}));
  }

  static Future<CyberCafeDetailModel> storeDetail(int id) async {
    var response = await http.get('/sideQuest/app/stores/stores/info',
        queryParameters: ({
          'id': id,
        }));
