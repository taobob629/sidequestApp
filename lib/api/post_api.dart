...
class PostApi {
  ...
  //Social-发布post
  static Future releasePost(
      {content = "", images = const [], var type = TYPE_DEFAULT}) async {
  ...
  /// Social-Post  帖子列表
  static Future<List<PostCommentModel>> getFavoratorsList(
      {int postsId = 0, int page = 0}) async {
    var response =
        await http.get('/peiwan/app/posts/praise/list', queryParameters: {
      "postsId": postsId,
      "pageNum": page,
      "pageSize": 20,
    });
  ...
  /// Social-Post  帖子列表
  static Future<List<PostCommentModel>> getPostCommentsList(
      {int postsId = 0, int page = 0}) async {
    var response = await http
        .get('/peiwan/app/posts/no/log/comment/list', queryParameters: {
      "postsId": postsId,
      "pageNum": page,
      "pageSize": 20,
    });
  ...
  static Future<PostItemModel> getPostDetail({required postsId}) async {
    var response = await http
        .get('/peiwan/app/posts/detail', queryParameters: {"postsId": postsId});
  ...
  static Future praisePost({required int postsId}) async {
    var response = await http
        .get('/peiwan/app/posts/praise', queryParameters: {"postsId": postsId});
    return response.data;
  }
