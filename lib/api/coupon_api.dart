...
class CouponApi {
  static Future<List<CouponModel>> list({int couponType = 0, int tab = CouponPage.TYPE_STORE}) async {
    var response = await http.get('/app/coupon/list', queryParameters: ({"couponType": couponType, 'tab': tab}));
    List<CouponModel> list = response.data.map<CouponModel>((item) => CouponModel.fromJson(item)).toList();
    return list;
  }
  ...
  static Future<List<CouponModel>> avaiPwcoupons(Map<String, dynamic>? preOrder) async {
    var preModel = preOrder;
    preModel?.removeWhere((key, value) => key != "preOrdersBos");
    var response = await http.post('/peiwan/app/new/orders/pwcoupons', data: preOrder);
    List<CouponModel> list = response.data.map<CouponModel>((item) => CouponModel.fromJson(item)).toList();
    return list;
  }

  static Future<String?> add(String code) async {
    var response = await http.get('/app/coupon/add', queryParameters: ({"code": code}));
