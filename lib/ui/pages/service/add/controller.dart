...
import 'package:sq_hub_app/ui/pages/service/add/pro/view.dart';
import 'package:sq_hub_app/ui/pages/service/add/service_type/view.dart';
...
import '../../hubs/game_home_page.dart';
import '../../profile/crop_page.dart';
import '../../profile/other_profile/record/controller.dart';
import '../skill/list/controller.dart';
import 'bio/view.dart';
...
//    mPriceRanges.addAll(serviceModel?.serviceTypes ?? []);
  ...
  toAddServiceTypePage() {
    Get.to(() => AddServiceTypePage(), arguments: game?.name);
  }
  ...
  void jumpProOrSubmit(String desc) {
    ...
    http.post('/peiwan/app/service/addService', data: data).then((v) {
      ...
      if (isEdit) {
        ...
        Get.until((route) => Get.currentRoute.contains("/skillList"));
  ...
  toBioPage() {
    ...
    Get.to(() => BioPage(), preventDuplicates: false)?.then((refresh) {
      if (refresh) {
        if (isEdit) onRefresh();
      }
    }).catchError((e) {
