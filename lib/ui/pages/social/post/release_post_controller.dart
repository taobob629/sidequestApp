...
import 'package:sq_hub_app/api/common.dart';
import 'package:sq_hub_app/api/post_api.dart';
...
class ReleasePostController extends GetxController {
  ...
  pickUploadPhoto() async {
    ...
    if (files.length > 9 || (photoList.length + files.length) > 9) {
      showToast('only 9 pictures allowed'.tr);
      return;
    }
  ...
  submit() {
    if (textController.text.trim().isEmpty) {
      showToast('Please enter content'.tr);
      return;
    }
    showLoading();
