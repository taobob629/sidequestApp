...
class ForgetPageController extends GetxController {
  ...
  void savePassword() async {
    ...
    bool flag = false;
    if (type == 1) {
      flag = await AuthApi.reset(email, password, code, uid);
    } else {
      flag = await AuthApi.resetPin(email, password, code, uid);
    }
    if (flag) {
      await showSuccess(
          "${'Your'.tr} ${type == 1 ? 'password'.tr : 'pin'.tr} ${'has been successfully reset!'.tr}",
          duration: Duration(seconds: 2));
      Get.back();
    }
  }
