        ...
        body: Column(
          children: [
            Container(
              ...
              child: Stack(
                children: [
                  Container(color: Color(0xFF141517), height: 11.h),
                  Container(color: hexColor('#303030'), height: 1.h),
                ],
              ),
            ),
            Expanded(
...
class OtherProfileController extends BasePageController {
  ...
  followOrNot(context, offset) {
    ...
    showLoading();
    UserApi.attention(player.value.memberId)
        .then((value) async {
          dismissLoading();
  ...
  editService(GamesItem game, ServiceItem serviceItem, String discount) async {
    if (isSelf) {
      ...
      Iterable<SkillModel> skillModel = list.where(
