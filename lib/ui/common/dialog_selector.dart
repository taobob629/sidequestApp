...
class SelectorDialog extends StatelessWidget {
    ...
    return Container(
      ...
      child: ListView.separated(
                ...
                children: [
                  Padding(
                    ...
                    child: Text(
                      item.displayLabel(),
                      style: TextStyle(
                        color: item.selectable() ? Colors.white : Colors.grey,
                        fontSize: 14
                      ),
                    ),
                  ),
        ...
        separatorBuilder: (context, index){
          return Divider(height: 1,color: Colors.white24,);
        },
  ...
  static Future<SelectorItem?> show(BuildContext context, List<SelectorItem> items, {String title = ""}) async {
    return await showDialog<SelectorItem>(
      context: context,
      barrierColor: Colors.black26,
      barrierDismissible: true,
      builder: (BuildContext context) {
        return SelectorDialog(items: items, title: title,);
      }
    );
  }
}