...
class ProfileHeader extends StatelessWidget {
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(15), gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Color(0x66171525), Colors.transparent])),
            ),
            Positioned(
                left: 15,
                top: 15,
            ...
            Positioned(
              ...
              child: GestureDetector(
                onTap: () {
                  ...
                  controller.checkLogin(() => Get.to(() => NavigatorHelper.toOtherProfile(controller.userInfoModel.value.pwuserId)));
                  // }else{
                  //   controller.checkLogin(()=>NavigatorHelper.gotoEditProfilePage());
                  // }
                },
                child: CircleAvatar(backgroundColor: Colors.white, radius: 40, child: Padding(padding: const EdgeInsets.all(2.0), child: _buildAvatar())),
              ),
            ),
            Positioned(
                left: 0,
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      if (controller.user.value.id != 0)
                        PlayLevelWidget(
                          isauth: controller.userInfoModel.value.isauth,
                          level: '${controller.userInfoModel.value.level}',
                          userId: controller.userInfoModel.value.pwuserId.toString(),
                        ),
                      //  _buildGradeWidget()
                    ],
                  ),
                ...
                child: Row(
                  ...
                  children: [
                    Text(
                      ...
                      style: TextStyle(color: Colors.white38, fontSize: 12, fontWeight: FontWeight.w300),
                    )
                  ],
                )),
            Positioned(
                  ...
                  children: [
                    ...
                    CountInfo(
                      ...
                      onTap: () => controller.checkLogin(
                        () => profilePageController.online.value ? Get.toNamed(AppPages.WALLET_PAGE, arguments: Map()..['page'] = 1) : null,
                      ),
                      icon: '',
                    ),
                    CountInfo(
            ...
            Positioned(
                ...
                right: 0,
                bottom: 0,
                child: profilePageController.online.value
  ...
  Widget _buildLevelIcon() {
    if (controller.userInfoModel.value.vipLevel == 0) {
      ...
      return Image.asset(
        "assets/images/ic_level${controller.userInfoModel.value.vipLevel}.webp",
        width: 26,
        height: 26,
      );
    }
  ...
  List<Widget> _buildVipViews() {
    ...
    for (int i = 0; i < profilePageController.vipInfoList.length; i++) {
      ...
      list.add(
        VipView(title: infoModel.name, icon: "ic_level${infoModel.level}", level: infoModel.level, index: i, fee: infoModel.monthFee),
      );
      list.add(
        SizedBox(width: 20),
      );
